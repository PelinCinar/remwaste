    ...
    <header className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur-sm border-b border-slate-700 text-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        ...
        <div className="relative">
          {/* Progress Bar Background - Bu bar tüm görünümlerde yer alıyor */}
          {/* Mobile'da da görünecek, eğer tamamen kaldırmak isteniyorsa conditional rendering eklenmeli */}
          <div className="absolute top-6 left-0 w-full h-1 bg-slate-700 rounded-full hidden lg:block"> {/* Sadece büyük ekranlarda göster */}
            {/* Progress Bar Fill */}
            <div
           ...
           {/* Tablet için ayrı bir progress bar */}
           <div className="absolute top-6 left-0 w-full h-1 bg-slate-700 rounded-full hidden md:block lg:hidden">
            <div
              className="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-500 ease-in-out shadow-lg shadow-blue-500/30"
              style={{ width: `${progressPercentage}%` }}
            ></div>
            ...
            {steps.map((step) => (
              <div key={step.id} className="flex flex-col items-center">
                ...
                >
                  ...
                  <step.icon
                    className={`w-6 h-6 ${                      step.completed ? 'text-white' :
            ...
            {steps.map((step) => (
              <div key={step.id} className="flex flex-col items-center">
                ...
                >
                  {/* Tablet Icons */}
                  <step.icon
                    className={`w-5 h-5 ${                      step.completed ? 'text-white' :
                      step.current ? 'text-white' :
                      'text-slate-400'
                    }`}
                  />
              ...
              {steps.map((step) => (
                <div key={step.id} className="flex flex-col items-center">
                  ...
                  >
                    {/* Mobile Icons */}
                    <step.icon
                      className={`w-7 h-7 ${                        step.completed ? 'text-white' :
                        step.current ? 'text-white' :
                        'text-slate-400'
                      }`}
                    />
